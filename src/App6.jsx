...
export default function App6() {
  ...
  return <main className={`app-shell ${resize ? 'is-resizing' : ''}`} onMouseMove={(ev) => { moveDrag(ev); moveResize(ev); }} onMouseUp={stopMove} onMouseLeave={stopMove}>
    ...
    <section className="preview-zone">{active.map((page, order) => <div className={`a4-page exam-page ${page === 0 ? '' : 'second-page'} ${exporting ? 'is-exporting' : ''} ${exporting && !pdfLines ? 'no-pdf-lines' : ''}`} key={page} ref={(n) => { pageRefs.current[page] = n; }}>{page === 0 && <header className="exam-header three-cell-header"><div className="header-cell left-header-cell class-duration-header"><textarea className="inline-class-input" value={level} onChange={(e) => setLevel(e.target.value)} rows="1" style={{ fontSize: `${classSize(level)}px` }} /><div className="tiny-duration-control"><button onClick={() => setDuration((d) => clamp(d - 1, 0, DURATIONS.length - 1))}>−</button><strong>{DURATIONS[duration]}</strong><button onClick={() => setDuration((d) => clamp(d + 1, 0, DURATIONS.length - 1))}>+</button></div></div><div className="header-cell middle-header-cell split-title-cell"><textarea className="inline-title-input title-line title-line-top" value={titleTop} onChange={(e) => setTitleTop(e.target.value)} rows="1" style={{ fontSize: `${fitSize(titleTop)}px` }} /><textarea className="inline-title-input title-line title-line-middle" value={titleMiddle} onChange={(e) => setTitleMiddle(e.target.value)} rows="1" style={{ fontSize: `${fitSize(titleMiddle)}px` }} /><textarea className="inline-title-input title-line title-line-bottom" value={titleBottom} onChange={(e) => setTitleBottom(e.target.value)} rows="1" /></div><div className="header-cell right-header-cell split-right-cell"><textarea className="inline-prof-input right-line right-line-top" value={rightTop} onChange={(e) => setRightTop(e.target.value)} rows="1" style={{ fontSize: `${fitSize(rightTop)}px` }} /><textarea className="inline-prof-input right-line right-line-bottom" value={rightBottom} onChange={(e) => setRightBottom(e.target.value)} rows="1" style={{ fontSize: `${fitSize(rightBottom)}px` }} /></div></header>}{renderList(page)}<div className="page-number">Page {order + 1}/{active.length}</div></div>)}</section>
  </main>;
}
